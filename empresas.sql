...
FOREIGN KEY (departamento_empresa_id)REFERENCES departamento_empresas (id_departamento_empresa) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO empleados(id_empleados, nombre, apellido, fecha_nacimiento, fecha_contratacion, telefono, tipo_empleado_id,depto_emp_oficina_id) 
...
SELECT nombre, apellido, fecha_nacimiento, YEAR(CURRENT_DATE) - YEAR(fecha_nacimiento) AS "edad" FROM empleados ORDER BY(nombre) desc ;
...
FROM telefonos AS t INNER JOIN depto_emp_oficinas AS deo INNER JOIN oficinas AS o INNER JOIN departamentos_guate AS dg INNER JOIN regiones AS r
ON t.id_telefono=deo.telefono_id AND deo.oficina_id=o.id_oficina AND o.departamento_guate_id=dg.id_departamento_guate AND dg.region_id=r.id_region
...
FROM telefonos AS t INNER JOIN depto_emp_oficinas AS deo INNER JOIN oficinas AS o INNER JOIN departamentos_guate AS dg INNER JOIN empleados AS e
