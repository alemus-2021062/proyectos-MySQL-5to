...
-- -----------------------------------------------------
DROP DATABASE IF EXISTS db_control_ventas_moises_lemus;
CREATE DATABASE IF NOT EXISTS db_control_ventas_moises_lemus;
USE db_control_ventas_moises_lemus;
...
FOR EACH ROW
BEGIN
	UPDATE productos SET precio_unitario =  NEW.total / NEW.cantidad 
    WHERE NEW.producto_id = productos.id;
    UPDATE productos SET precio_docena =  NEW.total / NEW.cantidad 
    WHERE NEW.producto_id = productos.id;
	UPDATE productos SET precio_mayor =  NEW.total / NEW.cantidad 
    WHERE NEW.producto_id = productos.id;
END $$
DELIMITER ;
...
DELIMITER $$
DROP TRIGGER IF EXISTS tr_detalle_compras_after_insert_2 $$
CREATE TRIGGER tr_detalle_compras_after_insert_2
AFTER INSERT
ON detalle_compras
FOR EACH ROW
BEGIN
